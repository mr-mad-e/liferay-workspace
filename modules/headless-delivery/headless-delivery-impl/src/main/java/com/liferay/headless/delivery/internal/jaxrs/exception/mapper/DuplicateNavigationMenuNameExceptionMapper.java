/**
 * SPDX-FileCopyrightText: (c) 2000 Liferay, Inc. https://liferay.com
 * SPDX-License-Identifier: LGPL-2.1-or-later OR LicenseRef-Liferay-DXP-EULA-2.0.0-2023-06
 */

package com.liferay.headless.delivery.internal.jaxrs.exception.mapper;

import com.liferay.portal.vulcan.jaxrs.exception.mapper.BaseExceptionMapper;
import com.liferay.portal.vulcan.jaxrs.exception.mapper.Problem;
import com.liferay.site.navigation.exception.DuplicateSiteNavigationMenuException;

import jakarta.ws.rs.core.Response;
import jakarta.ws.rs.ext.ExceptionMapper;

import org.osgi.service.component.annotations.Component;

/**
 * Converts any {@code DuplicateNodeNameException} to a {@code 400} error.
 *
 * @author Javier Gamarra
 */
@Component(
	property = {
		"osgi.jaxrs.application.select=(osgi.jaxrs.name=Dev.Liferay.Headless.Delivery)",
		"osgi.jaxrs.extension=true",
		"osgi.jaxrs.name=Dev.Liferay.Headless.Delivery.DuplicateNavigationMenuNameExceptionMapper"
	},
	service = ExceptionMapper.class
)
public class DuplicateNavigationMenuNameExceptionMapper
	extends BaseExceptionMapper<DuplicateSiteNavigationMenuException> {

	@Override
	protected Problem getProblem(
		DuplicateSiteNavigationMenuException
			duplicateSiteNavigationMenuException) {

		return new Problem(
			Response.Status.CONFLICT,
			"A navigation menu already exists with the name " +
				duplicateSiteNavigationMenuException.getMessage());
	}

}